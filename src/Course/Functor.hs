...
-- Note for the future self. We need a value of type k b and we have a value of type k (a -> b).
-- We can use fmap :: (k (a -> b) -> b) -> k (a -> b) -> k b; now we are left to find a function k :: (a -> b) -> b, but since we have
-- a value of type a, we can define k g = g a
