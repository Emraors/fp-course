...
betweenCharTok cl cr = between (charTok cl) (charTok cr)
...
hex = f =<< replicateA 4 (satisfy isHexDigit)
  where f c = case chr <$> readHex c of
          Full c' -> pure c'
          Empty -> constantParser (UnexpectedString c)
...
betweenSepbyComma cl cr pa = betweenCharTok cl cr (sepby pa commaTok)
