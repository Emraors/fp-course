...
newtype StateT s k a = StateT (s -> k (a, s))
...
instance Functor k => Functor (StateT s k) where
  ...
  (<$>) f state = let h = (<$>) (\(a,s) -> (f a,s))
                      g = runStateT state in
                  StateT (h . g )
...
instance Monad k => Applicative (StateT s k) where
  ...
  (<*>) fsab sa = StateT h
    where h s = runStateT fsab s >>= (\(f,s1) -> runStateT sa s1 >>= (\(a, s2) -> pure (f a, s2)))

-- Remark. For k = Id StateT s k a is just State a s. So I took the definition of <*> of
...
instance Monad k => Monad (StateT s k) where
  ...
  (=<<) f sa = StateT h
    where h s = runStateT sa s >>= (\(a, s1) -> runStateT (f a) s1)

-- Remark. As before I just add a do to the bind definition of the State monad
...
distinctF xs = evalT (filtering pn xs) S.empty
  where pn a = StateT (\s -> if a > 100
                             then Empty
...
instance Monad k => Applicative (OptionalT k) where
  ...
  (<*>) kof koa = (<$> koa) =<< kof
    --OptionalT ((<*>) <$> runOptionalT kof <*> runOptionalT koa)

    --(<$> koa) =<< kof
     -- OptionalT (lift2 (<*>) (runOptionalT kof) (runOptionalT koa))
    -- OptionalT (runOptionalT kof >>= (\of -> runOptionalT koa >>= (\oa -> pure (of <*> oa))))
...
instance Monad k => Monad (OptionalT k) where
  ...
  (=<<) f kopt = OptionalT (runOptionalT kopt >>= (\opta -> case opta of
                                                              (Full a) -> runOptionalT (f a)
                                                              Empty -> pure Empty))
--    error "todo: Course.StateT (=<<)#instance (OptionalT k)"

lambda :: Monad m => Optional (m a) -> m (Optional a)
lambda Empty = pure Empty
lambda (Full ma) = ma >>= (pure . Full)
...
instance Monad (Logger l) where
  ...
  (=<<) f (Logger xs a) = Logger xs id <*> f a
...
distinctG xs = runOptionalT (evalT (filtering pn xs) S.empty)
  where pn a = StateT (\s -> let val = Full (S.notMember a s, S.insert a s) in
                               case () of
                                 _ | a > 100 -> OptionalT (log1 ("aborting > 100: " ++ show' a) Empty)
                                   | even a -> OptionalT (log1 ("even number: " ++ show' a) val)
                                   | otherwise -> OptionalT (Logger Nil val))
