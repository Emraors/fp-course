...
convertInteractive =
  putStrLn "Enter the string you want to convert to upper-case: " >-
...
reverseInteractive =
  putStrLn "Enter the file you want to reverse: " >-
  getLine >>= \fileName ->
  putStrLn "Enter the file name to write the reversed file to " >-
  getLine >>= \finalFile ->
  readFile fileName >>= \fileContent ->
  writeFile finalFile (reverse fileContent)
...
urlEncoder :: Chars -> Chars
urlEncoder Nil = Nil
urlEncoder ('\t':.rest) = "%09" ++ urlEncoder rest
urlEncoder (' ':.rest) = "%20" ++ urlEncoder rest
urlEncoder ('\"':.rest) = "%22" ++ urlEncoder rest
urlEncoder xs = xs

encodeInteractive :: IO ()
encodeInteractive =
  putStrLn "Enter the url you want to encode: " >-
  getLine >>= (putStrLn . urlEncoder)
