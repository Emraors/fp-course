...
-- Starting from the bind operation >>= :: m a -> (a -> m b) -> mb and an element of type m (a -> b) we can produce an element of type
-- m b by applying bind to an element of type (a -> b) -> m b. In order to get such an element we can use the element of type m a we
-- are given and using fmp in this way: \f -> fmap f ma
...
-- Same as seqOptional but for a general Applicative
...
filtering p = foldRight (\a xs -> lift3 go (p a) (pure a) xs) (pure Nil)
  where go b a l = if b then a:.l else l
